    ...
    function initAutoArchivingWorker() {
        ...
        function AutoArchivingWorker() {
            ...
            if (isTop) {
                ...
                var self = this;
                fetch(location.protocol + '//' + wb_info.proxy_magic + '/proxy-worker')
                    .then(function (res) {
                        return res.text().then(function (text) {
                            ...
                            if ($wbwindow.document.readyState === "complete") {
                                ...
                                setInterval(self.checkIntervalCB, 30000);
                            } else {
                ...
                setInterval(this.checkIntervalCB, 30000);
            }
        }
            ...
            for (; i < styleSheets.length; ++i) {
                ...
                if (!this.shouldSkipSheet(sheet)) {
                    try {
                        ...
                        media = media.concat(this.extractMediaRules(sheet.cssRules, sheet.href || $wbwindow.document.baseURI));
                    } catch (error) {
